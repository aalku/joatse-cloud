...
```bash
export APP_INITIALIZATION_DATABASE_INIT_JSON_FILE=/app/data/database-init.json
```
...
> **Security Note**: Only regular users can be created via JSON initialization. Admin users must be created through other means. The `admin` user is automatically created and the password is stored in `joatse_admin_temp_password.txt`.
...
services:
  joatse-cloud:
    environment:
      - APP_INITIALIZATION_DATABASE_INIT_JSON_FILE=/app/data/database-init.json
    volumes:
      - ./data:/app/data
```
