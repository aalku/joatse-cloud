	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		Customizer<OAuth2LoginConfigurer<HttpSecurity>> oauth2LoginCustomizer = new Customizer<OAuth2LoginConfigurer<HttpSecurity>>() {
			...
			public void customize(OAuth2LoginConfigurer<HttpSecurity> x) {
				if (clientRegistrationRepository != null) {
					...
					x.defaultSuccessUrl(PATH_LOGGED_IN, true); // Removing it makes /form visible
					// .loginProcessingUrl("/loginPost"); // breaks form login
				} else {
					x.disable();
				}
			}
		...
		http.csrf().disable()
			.authorizeRequests()
				...
				.antMatchers(PATH_LOGIN_FORM, PATH_LOGIN_POST).permitAll()
				.antMatchers("/CF", "/CF/2").permitAll()
				.anyRequest().fullyAuthenticated()
        	.and()
        	.formLogin(c->{
        		c.loginPage(PATH_LOGIN_FORM).loginProcessingUrl(PATH_LOGIN_POST).defaultSuccessUrl(PATH_LOGGED_IN, true);
        	})
